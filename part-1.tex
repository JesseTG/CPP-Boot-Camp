...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    ...
    \draw (4cm, 21em) node  {Conditional compilation (you can also add\\\verb|#define|s through your build process)} -> \refer{cpp_define};
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    ...
    \draw (3cm, 24em) node  {Access modifiers same as in Java\\(but no package-private)} -> \refer{class_access_a};
    ...
    \draw (6cm, 4em) node [anchor=north west] {Marks method as an override} -> \refer{class_override};
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    \draw (3cm, 27em) node  {STL does this class better} -> \refer{raii_dont};
    \draw (5cm, 23em) node [anchor=south] {Member initialization syntax} -> \refer{raii_init};
    \draw (8cm, 22em) node [anchor=south west] {Other code} -> \refer{raii_ctor};
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{itemize}
    ...
    \item \cppinline{FormattedOutputFunction} $\rightarrow$ Function that outputs an object to a \cppinline{std::ostream} (like \cppinline{std::cout})
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    \draw (4cm, 17em) node  {\verb|sto(i|u?ll?|f|l?d)| is available, too} -> \refer{stoi_stoi};
    \draw (4cm, 12em) node  {Or \verb|to_wstring|} -> \refer{stoi_wstring};
    \draw (4cm, 10em) node  {\verb|a == 42|} -> \refer{stoi_a};
    \draw (7cm, 8em) node  {\verb|b == 9|\\(strips whitespace, reads as base-16)} -> \refer{stoi_b};
    \draw (8cm, 3em) node [anchor=north] {\verb|c == 2|\\\verb|first_nonnumeric_char_index == 2|} -> \refer{stoi_c};
    \draw (4cm, 3em) node  {\verb|"12"|} -> \refer{stoi_back};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    ...
    \draw (3cm, 1em) node [anchor=north] {Or use any \verb|UniformRandomNumberGenerator|\\(including \verb|random_device|, but it's slow)} -> \refer{random_concept};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    \draw (4cm, 16em) node  {Changes how \verb|float|s are printed out\\(many modifiers exist)} -> \refer{formatted_modifier};
    \draw (4cm, 12em) node  {\verb|std::ostream| that writes to a \verb|string|} -> \refer{formatted_stringstream};
    \draw (5cm, 2em) node [anchor=north west] {Use \verb|stringstream| for nontrivial formats (a la \verb|printf|)\\Use \verb|operator+| for concatenation\\Use \verb|sto(i|u?ll?|f|l?d)| for number conversions} -> \refer{formatted_str};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{tikzpicture}[callout]
    \draw (4cm, 23em) node  {Threading is complicated} -> \refer{time_thread};
    \draw (6cm, 20em) node  {Convert between time units} -> \refer{time_cast};
    \draw (7cm, 17em) node  {Really a \verb|typedef| for a generic duration} -> \refer{time_duration};
