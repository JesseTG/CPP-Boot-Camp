...
\begin{frame}[fragile=singleslide]
  ...
  \begin{figure}
    \begin{tikzpicture}
      \draw [fill=red, fill opacity=0.5, blend mode=multiply] (16,8) circle (3.8cm) node [align=center, anchor=center, fill opacity=1] {\huge{\textbf{C++}}};
      \draw [fill=blue, fill opacity=0.5, blend mode=multiply] (12.5,8) circle (.45cm) node [align=center, anchor=center, fill opacity=1] {\small{\textbf{C}}};
    \end{tikzpicture}
  \end{figure}
\end{frame}
...
int main() { 
  ...
  \begin{tikzpicture}[remember picture, ->, >=stealth, overlay, red, ultra thick, align=left]
    ...
    \draw (5cm, 5em) node [anchor=west] {\shortstack{Avoid this if the number may\\not be within the enum}} -> ([shift={(0em,.25em)}]pic cs:enum_ub);
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{itemize}
    ...
    \item Dealing with multiple files at once
    \item Using Visual Studio
    \item Overview of popular C++ tools
    \item Building and using Box2D
    \begin{itemize}
      \item This is part of your homework!
    \end{itemize}
  \end{itemize}
\end{frame}
